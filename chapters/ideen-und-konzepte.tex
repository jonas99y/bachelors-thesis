...
und auf irgendeine Weise dem Endbenutzer dargestellt werden.
...
Die Architekturidee ist, die Applikation in grob in drei Hauptteile zu unterteilen (Abbildung \ref{fig:high-level-arch}):

\begin{itemize}
    \item \texttt{dataingress}: Bietet eine Schnittstelle für die Stromzähler an, damit die von ihnen gesendeten Daten
        empfangen werden können.
    \item \texttt{api}: Stellt eine Schnittstelle zur verfügung über die Endbenutzer die Stromzählerdaten abfragen können.
    \item \texttt{datenbank}: Dient als indirekte Schnittstelle zwischen \texttt{dataingress} und \texttt{api}.
\end{itemize}

Dadurch, dass der dataingress nur in die Datenbank schreibt und die api nur lesend auf die Stromzählerdaten zugreift\footnote{
    Die api greift zwar bei der Erstellung der Labels schreibend auf die Datenbank zu, jedoch nicht auf von den
    Stromzählern gesendeten Daten.
},
