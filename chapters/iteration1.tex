...
Diese sollen empfangen, gespeichert und für Benutzer visualisiert werden.

Um dieses Ziel zu erreichen, mussten auch grundlegende Arbeiten, wie das Evaluieren von Technologien und Einrichten des \ac{CI/CD}
...
\end{itemize}
Die Entscheidung fiel auf eine Webseite.
Dies aus folgenden Gründen:
\begin{itemize}
    \item Das Projektteam hat in der Vergangenheit bereits Webseiten entwickelt.
          Diese Erfahrungen waren positiv und zeigten, dass so in geringer Zeit gute Resultate erreicht werden können.
    \item Eine Webseite schränkt die Zielplattform weniger ein als eine App.
          Bei einer App müsste bereits zu Beginn eine einzelne Plattform, wie bspw. Android oder iOS ausgewählt werden.
    ...
    \item Eine Person des Projektteams besuchte parallel zu diesem Projekt das Modul ``Datenvisualisierung``  an der Hochschule Luzern.
          ...
          Da mit D3.js Grafiken für Webseiten erstellt werden können, lag es auf der Hand, das Gelernte auch in dieser Arbeit umzusetzen.
\end{itemize}
...
Die Entscheidung fiel auf React.js, da dieses sich für Anfänger eignet und im Zusammenspiel mit weiteren Frameworks eingesetzt werden kann \parencite{react_angular_vue}.
Letzteres ist wichtig, da das zuvor erwähnte D3.js Framework für die Visualisierungen verwendet werden soll.

Um die Entwicklung des Frontends durch das Verwenden von vorgefertigte Komponenten zu erleichtern, können entsprechende Frameworks eingesetzt werden.
...
\subsection{Backend}
Auch bei der Auswahl des Backend Technologiestacks wurde auf verschiedene Aspekte geachtet.
...
Python bietet für jede dieser Anforderungen gleich mehrere Frameworks an. Mit Python können sehr
...
Um die Applikation für die Entwicklung lokal aufzusetzen, zu testen oder zu deployen, wurde
...
und das Datenbankschema kann zentral in der \texttt{datastore} Library einmal definiert
...
\begin{itemize}
    ...
    \item \texttt{datastore}:
          Der \texttt{datastore} ist eine Python Library, die das Datenbankschema mittels \ac{ORM} festlegt.
          Sowohl die \ac{API} als auch der \texttt{dataingress} greifen auf die Datenbank zu
          und verwenden somit die \texttt{datastore} Library.

    \item \texttt{dataingress}:
          Der \texttt{dataingress} arbeitet die von den \ac{IoT} Geräten gesendeten
...
Dies, da auch die \texttt{paho} Python \ac{MQTT} Library vom gleichen Entwickler Eclipse verwendet wird.
...
Die Abfrage landet beim Nginx und wird an das Python FastAPI Backend weitergeleitet.
Dieses Python Backend führt danach eine SQL Abfrage via \ac{ORM} auf der Datenbank aus.
Um die Daten im Web Frontend anzeigen zu können, werden sie nach \ac{JSON} serialisiert
und zurück an die WebApp gesendet.

Die \texttt{datastore} Python Library stellt der \ac{API} das Datenbankschema zur Verfügung,
...
Da nun das Datenmodell (in der \texttt{datastore} Library) und die \ac{API} definiert wurden,
kann das Web Frontend unabhängig vom Verarbeiten der \ac{MQTT} Daten gemacht werden.

\subsection{Implementation der Benutzerschnittstelle}
\begin{figure}
    \centering
...
Abbildung \ref{fig:mobileUI} zeigt wie die Benutzerschnittstelle nach der ersten Iteration ausgesehen hat.

Die zu visualisierenden Daten sind jeweils Messwerte von Spannung oder Leistung zu einem bestimmten Zeitpunkt.
Diese Daten sind nur dann aussagekräftig und interessant, wenn sie in zeitliche Abfolge dargestellt sind.
Deshalb wurden Liniendiagramme gewählt. Ein Vorteil dieser ist, dass verschiedene Werte, bspw. die drei Spannungswerte der verschiedenen Iteration
...
Per Datenbank \ac{URI} kann dem Sqlmodel-Framework eine Datenbankanbindung
mitgegeben, werden um zu bestimmen, auf welche Datenbank zugegriffen werden soll.
...
\begin{figure}[h]
    ...
    \caption{
        Architektur des \ac{MQTT} Messagebrokers und des \texttt{dataingress}, welcher die
        empfangenen Nachrichten in die Datenbank persistiert
    }
    \label{fig:arch-mqtt}
\end{figure}

Der \texttt{dataingress} (ebenfalls ein Python Poetry projekt) registriert sich dann auf die von
...
Auch der \texttt{dataingress} verwendet die \texttt{datastore} Python Library. Dies stellt sicher,
dass die \ac{API} und der \texttt{dataingress} das gleiche Datenbankformat verwenden, da
...
\begin{figure}[h]
    ...
    \caption{
        Das Sequenzdiagramm zeigt auf wie, der \texttt{dataingress} die \ac{MQTT} Messages
        von den Stromzählern abarbeitet und in die Datenbank persisitiert
    }
    \label{fig:dataingress-sequence-no-err}
\end{figure}

Im Sequenzdiagramm des \texttt{dataingress} (Abbildung \ref{fig:dataingress-sequence-no-err}) wird aufgezeigt,
wie \ac{MQTT} Messages vom \texttt{dataingress} abgearbeitet werden.
...
Um die Funktionalität und den Erfüllungsgrad der Anforderungen in der ersten Iteration zu verifizieren, wurde anhand
...
Durch die Hilfe von Freunden, Mitstudierenden und Hilfsmittel im Internet liessen sich diese meistern
